    ...
    <div className="relative min-h-screen flex flex-col overflow-x-hidden font-sans">
      ...
      <header className="sticky top-0 z-40 w-full glass-panel border-b border-white/5 py-4 px-6 md:px-12 flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center gap-8 font-serif text-sm tracking-widest text-[#eaeaea]/80">
          ...
          <a href="#trust" className="hover:text-gold transition-colors">BẢO CHỨNG</a>
        </nav>
        <div>
          <a
            href="#showroom"
      ...
      <main className="flex-grow z-10">
        {/* Hero Section */}
        <section className="relative min-h-[85vh] flex flex-col justify-center items-center text-center px-6 py-20">
          <div className="max-w-4xl space-y-6">
            <span className="text-xs tracking-[0.4em] uppercase text-gold font-mono block">
              TINH HOA NÔNG SẢN VÙNG MIỀN
            </span>
            <h1 className="font-serif text-5xl md:text-7xl lg:text-8xl tracking-wide leading-tight text-white font-light">
              Gói Trọn <span className="italic font-normal text-gold-accent font-serif">Ân Tình</span> Đất Việt
            </h1>
            <p className="max-w-2xl mx-auto font-sans text-sm md:text-base text-[#eaeaea]/70 font-light leading-relaxed">
              Mỗi thức quà tại Ocopia là một câu chuyện văn hóa, được nuôi dưỡng từ lòng đất quê hương và chế biến qua đôi bàn tay nghệ nhân gìn giữ truyền thống trăm năm.
            </p>
            <div className="pt-6">
              <a
                href="#showroom"
                className="inline-block font-serif text-sm tracking-widest bg-gradient-to-r from-gold via-gold-accent to-gold text-dark-bg hover:opacity-90 transition-all duration-300 font-semibold py-4 px-10 rounded-sm shadow-lg shadow-gold/10"
              >
                KHÁM PHÁ NGAY
              </a>
            </div>
          ...
          <div className="absolute bottom-10 flex flex-col items-center gap-2 text-gold-accent/50 animate-bounce">
            <span className="text-[10px] tracking-widest font-mono uppercase">Cuộn xuống</span>
        ...
        <section id="showroom" className="max-w-7xl mx-auto px-6 md:px-12 py-24 space-y-32">
          <div className="text-center space-y-4 max-w-2xl mx-auto">
            <span className="font-mono text-xs tracking-[0.3em] text-gold uppercase">SHOWROOM CHÍNH HÃNG</span>
          ...
          {loading ? (
            ...
            <div className="space-y-28">
              {products.map((product, idx) => {
                const { hook, core } = parseStory(product.story);
                const trustPoints = getProductTrustPoints(product.id);
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full lg:w-1/2 relative group">
                      <div className="absolute -inset-1 bg-gradient-to-r from-gold/30 to-gold-accent/10 rounded-lg blur opacity-25 group-hover:opacity-40 transition duration-1000"></div>
                      <div className="relative aspect-[4/3] rounded-lg overflow-hidden glass-panel border border-white/10">
                        <img
                          src={product.image_url}
                          alt={product.name}
                          className="w-full h-full object-cover transform group-hover:scale-105 transition-transform duration-700"
                        />
                        <div className="absolute top-4 left-4 bg-dark-bg/80 backdrop-blur-md border border-gold/30 px-3 py-1 rounded-sm text-[10px] font-mono tracking-widest text-gold-accent">
                          {product.id === 1 ? "OCOP 4 SAO" : "OCOP 3 SAO"}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="w-full lg:w-1/2 space-y-6">
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-white/5">
                        <div className="space-y-1">
                          ...
                          <p className="font-serif text-2xl text-gold-light font-medium">
                            {product.price.toLocaleString("vi-VN")} VNĐ
                          </p>
                        </div>
                        <button
                          onClick={() => openCheckout(product)}
                          className="font-serif text-xs tracking-widest bg-gold text-dark-bg hover:bg-gold-light transition-all duration-300 font-semibold py-3.5 px-8 rounded-sm"
                        >
                          MUA NGAY
                        </button>
                      </div>
      ...
      {isCheckoutOpen && activeProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
          ...
          {/* Modal Container */}
          <div className="relative w-full max-w-2xl glass-panel border border-white/10 rounded-md overflow-hidden z-10 max-h-[90vh] flex flex-col">
            {/* Modal Header */}
      ...
      <footer id="about" className="border-t border-white/5 py-12 px-6 md:px-12 bg-dark-bg/60 text-center space-y-6">
        <div className="max-w-2xl mx-auto space-y-4">
          ...
          <p className="font-sans text-xs text-[#eaeaea]/50 leading-relaxed font-light">
